...
class LatControlTorque(LatControl):
  def __init__(self, CP, CI):
    ...
    if self.use_nn or self.use_lateral_jerk:
      ...
      self.lat_jerk_friction_factor = 0.4 # old=0.4
      self.lat_accel_friction_factor = 0.7 # old =0.7 # in [0, 3], in 0.05 increments. 3 is arbitrary safety limit
    ...
    if self.use_nn:
      ...
      self.nn_time_offset = CP.steerActuatorDelay + 0.18
  ...
  def update(self, active, CS, VM, params, steer_limited, desired_curvature, llk, model_data=None):
    ...
    if not active:
      ...
      actual_curvature_vm = -VM.calc_curvature(math.radians(CS.steeringAngleDeg - params.angleOffsetDeg), CS.vEgo, (params.roll * rollEdit))
      roll_compensation = (params.roll *rollEdit) * ACCELERATION_DUE_TO_GRAVITY
      ...
      if self.use_nn and model_good:
        ...
        pid_log.error = (torque_from_setpoint - torque_from_measurement) * ERROR_PERCENT      # old =   pid_log.error = torque_from_setpoint - torque_from_measurement
        ...
        error = (setpoint - measurement) * ERROR_PERCENT     # old = error = setpoint - measurement
        ...
        pid_log.error = (torque_from_setpoint - torque_from_measurement) * ERROR_PERCENT      # old =  pid_log.error = torque_from_setpoint - torque_from_measurement
        error = (desired_lateral_accel - actual_lateral_accel) * ERROR_PERCENT    # old = error = desired_lateral_accel - actual_lateral_accel
